...
test('Components documentation renders semantic themes responsively without browser errors', async ({ page }, testInfo) => {
  ...
  await expect(statusListbox.getByRole('option', { name: 'Active' })).toBeFocused()
  ...
  await statusSelect.click()
  await expect(statusListbox.getByRole('option', { name: 'Pending' })).toBeFocused()
  await page.keyboard.press('End')
  await expect(statusListbox.getByRole('option', { name: 'Suspended' })).toBeFocused()
  await page.keyboard.press('Enter')
  await expect(statusSelect).toContainText('Suspended')
  await expect(statusSelect).toHaveAttribute('aria-expanded', 'false')
  await expect(choiceSurface.locator('input[type="hidden"][name="status"]')).toHaveValue('suspended')
  ...
  await expect(accountListbox.getByRole('option', { name: 'Operating' })).toBeFocused()
  await page.keyboard.press('End')
  await page.keyboard.press('Enter')
