...
test('Documentation App mode fixtures expand browser and phone surfaces with independent review states @cross-browser', async ({ page }) => {
  ...
  await expect(root.locator('[data-fve-browser-toolbar="true"]')).toHaveCount(0)
  ...
  const reviewState = controls.getByRole('button', { name: 'Review state' })
  ...
  const validationInLightDock = page.getByRole('menuitem', { name: 'Validation', exact: true })
  await expect(validationInLightDock).toBeVisible()
  await validationInLightDock.hover()
  await expect.poll(() => validationInLightDock.evaluate(element => getComputedStyle(element).color)).toBe('rgb(255, 255, 255)')
  ...
  await reviewState.click()
  const validation = page.getByRole('menuitem', { name: 'Validation', exact: true })
  ...
  await page.getByRole('menuitem', { name: 'Validation', exact: true }).click()
  ...
  await page.evaluate(() => window.fsharpDocsColorMode.set('dark'))
  await page.getByRole('link', { name: 'Open Create a view on phone in App mode' }).click()
...
test('Documentation page examples use wide fixtures, relevant building blocks, and App mode', async ({ page }) => {
  for (const item of [
    ...
    const launch = example.getByRole('link', { name: `Open ${item.label} in App mode` })
    const returnUrl = new URL((await launch.getAttribute('href'))!, page.url())
    returnUrl.searchParams.delete('fveAppMode')
    returnUrl.searchParams.delete('fveAppFrame')
    returnUrl.searchParams.delete('fveAppTransition')
    await launch.click()
    const root = page.locator('[data-fve-app-mode-root="true"]')
    const controls = page.locator('[data-fve-app-mode-controls="true"]')
    await expect(root).toHaveAttribute('data-fve-app-mode-surface', 'browser')
    ...
    expect(new URL(page.url()).pathname).toBe(returnUrl.pathname)
    expect(new URL(page.url()).search).toBe(returnUrl.search)
  }
})
...
test('Documentation page examples inherit the resolved host color mode in previews and App mode', async ({ page }) => {
  for (const [path, label] of [
    ...
    await example.getByRole('link', { name: `Open ${label} in App mode` }).click()
    const root = page.locator('[data-fve-app-mode-root="true"]')
    const controls = page.locator('[data-fve-app-mode-controls="true"]')
...
test('Documentation, API, and specification examples keep navigation inside their fixtures @cross-browser', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Open Executable specification page example in App mode' }).click()
...
test('Fixture keeps its exact compiled source beside preview and copy controls', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => (window as typeof window & { copiedFixtureSource?: string }).copiedFixtureSource)).toContain('Fixture.browser "checkout-workflow"')
})

test('Browser and Phone stay static while Fixture owns fullscreen review context @cross-browser', async ({ page }) => {
  const root = page.locator('[data-fve-app-mode-root="true"]')
  ...
  await page.getByRole('link', { name: 'Open Shipping address in App mode' }).click()
  await expect(root).toHaveAttribute('data-fve-app-mode-surface', 'browser')
  await expect(root.locator('[data-fve-browser-toolbar="true"]')).toHaveCount(0)
  ...
  await page.getByRole('link', { name: 'Open Saved offers in App mode' }).click()
  await expect(root).toHaveAttribute('data-fve-app-mode-surface', 'phone')
  await page.locator('[data-fve-app-mode-controls="true"]').getByRole('link', { name: 'Exit App mode' }).click()

  await page.goto('/docs/components/fixture?fixtureStep=shipping&fixtureState=ready')
  await page.getByRole('link', { name: 'Open Shipping address in App mode' }).click()
  const controls = page.locator('[data-fve-app-mode-controls="true"]')
  await expect(controls.getByRole('button', { name: 'Review state' })).toBeVisible()
  await expect(root).toContainText('Enter a delivery address to continue.')

  await controls.getByRole('link', { name: 'Previous: Cart' }).click()
  await expect(page).toHaveURL(/fixtureStep=cart.*fveAppMode=app/)
  await expect(root).toContainText('One item is ready for checkout.')
  await expect(controls.getByRole('link', { name: 'Previous:' })).toHaveCount(0)
  await controls.getByRole('link', { name: 'Next: Shipping address' }).click()

  await expect(page).toHaveURL(/fixtureStep=shipping.*fveAppMode=app/)
  await controls.getByRole('link', { name: 'Next: Payment' }).click()
  await expect(page).toHaveURL(/fixtureStep=payment.*fveAppMode=app/)
  await expect(root).toContainText('Choose a payment method to place the order.')
  await expect(controls.getByRole('link', { name: 'Next:' })).toHaveCount(0)
  ...
  await page.goto('/docs/components/fixture?fixtureStep=cart&fixtureState=validation&fveAppMode=app&fveAppFrame=checkout-workflow')
  await expect(root).toContainText('Review the quantity before continuing.')
  await controls.getByRole('link', { name: 'Exit App mode' }).click()
  await expect(page).toHaveURL('/docs/components/fixture?fixtureStep=cart&fixtureState=validation')
})
