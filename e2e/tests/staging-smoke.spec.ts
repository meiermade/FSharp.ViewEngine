...
const configuredOrigin = new URL(process.env.DOCS_E2E_BASE_URL ?? stagingOrigin).origin
...
if (configuredOrigin !== stagingOrigin) {
  throw new Error(`Staging smoke credentials may only target ${stagingOrigin}`)
}
if (!expectedCommit || !expectedImage || !accessClientId || !accessClientSecret) {
  throw new Error('Staging smoke requires commit, image, and Cloudflare Access service credentials')
}
