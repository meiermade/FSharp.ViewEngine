const retiredPackage = 'FSharp.ViewEngine.Docs'
const replacementPackage = 'FSharp.ViewEngine.Components'
...
const registrationEntries = async () => {
    ...
    return pages.flat().map(item => item.catalogEntry)
}

const availableVersions = (await getJson(versionsUrl)).versions
const entries = await registrationEntries()
