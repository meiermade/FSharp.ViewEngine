...
for (const node of existing.values()) node.remove();"""
    ...
    let selectAll actionLabel label signal unavailable eligibleChoices focusId =
        ...
        button {
            ...
            _class (ComponentHtml.classes [ ComponentHtml.popupControlClasses; "fve-popup-control inline-flex min-h-8 items-center rounded-[var(--fve-radius-control)] px-2 text-[length:var(--fve-control-font-size)] leading-[var(--fve-control-line-height)] font-medium text-[var(--fve-muted-text)] hover:text-[var(--fve-text)] disabled:cursor-not-allowed disabled:opacity-50" ])
            text actionLabel
        }
    ...
    let clear label signal initial unavailable focusId =
        ...
        button {
            ...
            _class (ComponentHtml.classes [ ComponentHtml.popupControlClasses; "fve-popup-control ml-auto inline-flex min-h-8 items-center rounded-[var(--fve-radius-control)] px-2 text-[length:var(--fve-control-font-size)] leading-[var(--fve-control-line-height)] font-medium text-[var(--fve-muted-text)] hover:text-[var(--fve-text)] disabled:cursor-not-allowed disabled:opacity-50" ])
            "Clear selection"
        }
