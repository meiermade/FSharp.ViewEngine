...
- `src/` — All F# source code, build system, and Docker build
- `etc/` — Assets (logos)
...
./fake.sh Test
dotnet run --project Tests/Tests.fsproj   # Direct

# Run a single test by name
dotnet run --project Tests/Tests.fsproj -- --filter "Should render html document"
...
### Core Type System (`src/FSharp.ViewEngine/Core.fs`)
...
### Module Organization (`src/FSharp.ViewEngine/`)
...
### Project Structure
- `src/FSharp.ViewEngine/` — Core library (NuGet package)
- `src/Tests/` — Expecto tests
- `src/Benchmarks/` — BenchmarkDotNet benchmarks
- `src/Build/` — FAKE build system (Test, Pack, PushNugets, Watch, BuildCss, Publish targets)
- `src/Docs/` — Documentation Giraffe web app with Markdown pages
...
- **New doc pages**: Add markdown in `src/Docs/docs/`, handler in `Handlers.fs`, route in `Program.fs`, nav link in `Views.fs`.
...
## Build System
- FAKE build script invoked via `fake.cmd`/`fake.sh` in `src/`
- Paket for package management (`paket.dependencies` in `src/`)
